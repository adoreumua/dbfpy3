...
from . import utils, DEFAULT_ENCODING
...
class DbfCharacterFieldDef(DbfFieldDef):
    ...
    def encodeValue(self, value):
        ...
        if type(value) not in [bytes]:
            value = bytes(str(value), DEFAULT_ENCODING)
...
class DbfNumericFieldDef(DbfFieldDef):
    ...
    def encodeValue(self, value):
        ...
        return bytes(_rv, DEFAULT_ENCODING)
...
class DbfDateFieldDef(DbfFieldDef):
    ...
    def encodeValue(self, value):
        ...
        """
        if value:
            return bytes(utils.getDate(value).strftime("%Y%m%d"), DEFAULT_ENCODING)
        else:
